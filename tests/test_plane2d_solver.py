...
class TestPlane2DSolver(unittest.TestCase):
    def test_cantilever_example(self):
        ...
        elements = [[1, 7, 8], [1, 2, 7], [2, 6, 7], [2, 3, 6], [3, 5, 6], [3, 4, 5]]
        ...
        a, r, es, ed, results_summary = solve_plane2d(
            coords, dofs, edofs, bdofs, material_data, boundary_conditions, loads
        )

        # Basic checks
        self.assertEqual(a.shape[0], coords.shape[0] * 2, "Displacement vector length mismatch.")
        self.assertFalse(
            np.allclose(a, 0), "All zero displacements suspicious for predefined mesh."
        )
