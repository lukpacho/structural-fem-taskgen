...
\begin{center}
    ...
    \noindent Dla przedstawionego układu w \textbf{\VAR{analysis_type}} należy wyznaczyć:
    \begin{itemize}
    ...
    \BLOCK{ for bc in bc_list}
        \VAR{bc.point1} oraz \VAR{bc.point2} \VAR{bc.direction}%
    \BLOCK{ endfor }. 
    ...
    \begin{figure}[ht]
        \centering
        \includegraphics[width=\textwidth]{\VAR{plot_path}}
    \end{figure}
    ...
    \item Wyniki przemieszczeń i naprężeń dla obu wariantów (uzupełnij jednostki w tabeli poniżej) \dotfill 5 pkt
    \begin{table}[ht]
        \centering
        \begin{tabular}{@{}m{.02\textwidth} m{.2\textwidth} m{.2\textwidth} m{.2\textwidth}@{}}
        \toprule
         & $u_x$ [\dotfill] & $v_y$ [\dotfill] & $max(\sigma)$ [\dotfill] \\ \midrule
         \textbf{A)} & & & \\ \midrule
         \textbf{B)} & & & \\ \bottomrule
        \end{tabular}
        \label{tab:rezultaty}
