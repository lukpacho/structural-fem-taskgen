...
\begin{document}
    ...
    \BLOCK{ for force in forces}
        \BLOCK{if loop.first}
            \BLOCK{ if loop.last > 1} sił skupionych
    ...
    \BLOCK{ for moment in moments}
        \BLOCK{if loop.first}
            \BLOCK{ if loop.last > 1} momentów zginających
    ...
    \BLOCK{ for lineload in lineloads}
        \BLOCK{if loop.first}
            \BLOCK{ if loop.last > 1} obciążeń liniowych
    ...
    \begin{enumerate}
    \item Rysunek układu po dyskretyzacji \dotfill 5 pkt
